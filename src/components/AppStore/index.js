...
class AppStore extends Component {
  state = {activeId: tabsList[0].tabId, serachInput: ''}
  ...
  getFilterProjects = () => {
    ...
    const filteredProjects = appsList.filter(each => each.category === activeId)
    return filteredProjects
  }

  render() {
    const {activeId, serachInput} = this.state
    const filteredProjects = this.getFilterProjects()
    const searchData = filteredProjects.filter(
      each =>
    ...
    return (
      <div className="background-container">
        ...
        <div className="app-list">
          <ul className="unorder-list">
            {searchData.map(each => (
              <AppItem listApps={each} key={each.appId} />
            ))}
          </ul>
        </div>
      </div>
